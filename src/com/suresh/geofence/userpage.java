...
import com.suresh.network.Receiver_Get;
...
public class userpage extends BaseActivity implements AdapterView.OnItemClickListener, View.OnClickListener
{
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
	    ...
	    {
	        fencename=mydata.getData("name");
		    status_db=mydata.getData("status");
		    type=mydata.getData("type");
		    lat=mydata.getData("lat");
		    lgt=mydata.getData("lgt");
		    rad=mydata.getData("radius");
		    message=mydata.getData("message");
		    catagory=mydata.getData("catagory");
		    shape=mydata.getData("shape");
		    
		    admin_fencename=mydata.getAdminData("name");
		    admin_type=mydata.getAdminData("type");
		    admin_lat=mydata.getAdminData("lat");
		    admin_lgt=mydata.getAdminData("lgt");
		    admin_rad=mydata.getAdminData("radius");
		    admin_message=mydata.getAdminData("message");
		    admin_catagory=mydata.getAdminData("catagory");
	        ...
	        try {
				...
				for(int i=0;i<report_lat.size();i++)
				{
					Log.i("lats", report_lat.get(i));
				}
